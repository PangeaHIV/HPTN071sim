read.example.Feb2015<- function()
{
	...
	file	<- '~/Dropbox (Infectious Disease)/PANGEAHIVsim_internal/documents/external/2015_05_results/PANGEA_Submission_Colijn.csv'
...
{
	require(data.table)
	cat('\nThis is read.submission version 15-05-26.')
	if(verbose)
	{
		cat('\nReminder of objectives\n(use verbose=0 to suppress this message)')
	...
	#	check that 3m<6m
	#
	tmp			<- df[, !is.na(OBJ_v_3m) & !is.na(OBJ_v_6m) & ESTIMATE=='central']
	if(length(tmp))
		invisible(df[tmp,][,{
	...
	#	check that 6m<12m
	#
	tmp			<- df[, !is.na(OBJ_v_6m) & !is.na(OBJ_v_12m) & ESTIMATE=='central']
	if(length(tmp))
		invisible(df[tmp,][,{
