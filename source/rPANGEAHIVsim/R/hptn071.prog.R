##--------------------------------------------------------------------------------------------------------
##	program to read sequences generated by Seq Gen 1.3.3
##	olli originally written 26-08-2014
##--------------------------------------------------------------------------------------------------------
prog.PANGEA.SeqGen.readOutput<- function()
{
	verbose				<- 1
	...
	if(verbose)
	{
		cat('\ninput args\n',paste(indir.epi, infile.epi, indir.sg, outdir, sep='\n'))
	}
	#
...
prog.PANGEA.SeqGen.createInputFile<- function()
{
	...
	#
	tmp			<- log.df[,  lapply(.SD, rank, ties.method='random'), by=c('GENE','CODON_POS')]	
	tmp			<- tmp[, 	{
				z		<- do.call('cbind',lapply( .SD, function(x) abs(x-length(meanRate)/2)))
				list(dist.from.midrank= apply(z, 1, sum) )								
			}, by=c('GENE','CODON_POS')]		
	tmp			<- tmp[, list(IDX=which.min(dist.from.midrank)), by=c('GENE','CODON_POS')]
	gtr.central	<- log.df[,  {
				z<- which( tmp$GENE==GENE & tmp$CODON_POS==CODON_POS )
				lapply(.SD, '[', tmp$IDX[z] )
			}, by=c('GENE','CODON_POS')]
	if(with.plot)
	{
		#plot selected GTR parameters per model
		tmp		<- melt(log.df, id=c('GENE','CODON_POS'))		
		tmp2	<- melt(gtr.central, id=c('GENE','CODON_POS'))				
		ggplot( tmp, aes(x=value)) + geom_histogram() + facet_grid(GENE+CODON_POS~variable, scales='free') +
				geom_vline(data=tmp2, aes(xintercept= value), colour="#D55E00") + labs(x='GTR parameter')
		file	<- paste(indir.epi, '/', substr(infile.epi,1,nchar(infile.epi)-6),'INFO_sg_GTRCENTRALparam.pdf', sep='')
		ggsave(file, w=20, h=15)							
	}
	#	scale relative substitution rate so that mean is 1	-- this is used to vary the substitution rates by gene and codon position
	set(gtr.central, NULL, 'mu', gtr.central[, mu/mean(mu)])
	if(with.plot)
	{
		#plot within-host evolutionary rate per model
		tmp		<- subset(gtr.central, select=c(GENE, CODON_POS, mu))
		tmp		<- tmp[,list(ER= mu*df.nodestat$ER), by=c('GENE', 'CODON_POS')]		
		...
		file	<- paste(indir.epi, '/', substr(infile.epi,1,nchar(infile.epi)-6),'INFO_sg_ERCENTRAL_by_MODEL.pdf', sep='')
		ggsave(file, w=6, h=6)
		#ggplot(tmp, aes(x=ER, colour=CODON_POS, group=CODON_POS)) + geom_density(aes(y = ..density..), weight=1.2) +
		#		stat_summary(fun.y=median, fun.ymin = median, fun.ymax = median, geom = "crossbar", width = 0.5) +
		#		facet_grid(GENE~., scales='free_y') +
		#		scale_x_continuous(breaks= seq(0, 0.05, 0.002)) + labs(linetype='Gene', colour='Codon\nPosition', x='simulated evolutionary rate')
	}	
	#
